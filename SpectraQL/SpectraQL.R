...
img <- readPNG("drawings/MassQL-hero.png")
...
## font_text <- "Aller_Lt"
## font_text <- "Aller_Rg"
## font_text <- "SourceCodePro-Bold"
## font_text <- "Inconsolata-Regular"
font_text <- "roboto"
## font_text <- "quicksand"
...
hex <- ggplot() +
    ...
    hex_segment2(size = 0, fill = paste0(col_gnps, 10), # top right
                 from_radius = 0, to_radius = 0.9,
                 from_angle = 30, to_angle = 90) +
    hex_segment2(size = 0, fill = paste0(col_gnps, 10), # top left
                 from_radius = 0, to_radius = 0.9,
                 from_angle = 90, to_angle = 150) +
    hex_segment2(size = 0, fill = paste0(col_orange, 10), # left
                 from_radius = 0, to_radius = 0.9,
                 from_angle = 150, to_angle = 210) +
    hex_segment2(size = 0, fill = paste0(col_orange, 10), # bottom
                 from_radius = 0, to_radius = 0.9,
    ...
    ## border
    hex_segment2(size = 0, fill = paste0(col_gnps, "ce"),
                 from_radius = 0.9, to_radius = 1,
                 from_angle = 330, to_angle = 30) +
    hex_segment2(size = 0, fill = paste0(col_gnps, "ff"),
                 from_radius = 0.9, to_radius = 1,
                 from_angle = 30, to_angle = 90) +
    hex_segment2(size = 0, fill = paste0(col_gnps, "ff"),
                 from_radius = 0.9, to_radius = 1,
    ...
    hex_segment2(size = 0, fill = paste0(col_gnps, "ce"),
                 from_radius = 0.9, to_radius = 1,
                 from_angle = 150, to_angle = 210) +
    hex_segment2(size = 0, fill = paste0(col_gnps, 80),
                 from_radius = 0.9, to_radius = 1,
                 from_angle = 210, to_angle = 270) +
    hex_segment2(size = 0, fill = paste0(col_gnps, 80),
                 from_radius = 0.9, to_radius = 1,
    ...
    geom_subview(subview = img, x = 1.0, y = 0.7,
                 width = 0.95, height = 0.95) +
    ## font size for linux: 6.5, macOS
    geom_url("https://rformassspectrometry.org", x = 0.99, y = 0.15,
             color = col_gnps, size = 4.5, family = font_text) + 
    geom_url("SpectraQL", x = 0.29, y = 1.3, angle = 0,
             color = col_gnps, size = 24, family = font_text) + 
    theme_sticker()
save_sticker(filename = "SpectraQL.png", hex)
