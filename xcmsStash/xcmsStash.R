...
img <- readPNG("drawings/xcmsStash-drawing.png")
...
hex <- ggplot() +
    ...
    ## border
    hex_segment2(linewidth = 0, fill = paste0(red, "ff"), # right
                 from_radius = 0.9, to_radius = 1,
                 from_angle = 330, to_angle = 30) +
    hex_segment2(linewidth = 0, fill = paste0(red, "ff"), # top right
                 from_radius = 0.9, to_radius = 1,
                 from_angle = 30, to_angle = 90) +
    hex_segment2(linewidth = 0, fill = paste0(col_gold, "ff"), # top left
                 from_radius = 0.9, to_radius = 1,
                 from_angle = 90, to_angle = 150) +
    hex_segment2(linewidth = 0, fill = paste0(red, "ff"), # left
                 from_radius = 0.9, to_radius = 1,
                 from_angle = 150, to_angle = 210) +
    hex_segment2(linewidth = 0, fill = paste0(red, "ff"), # bottom
                 from_radius = 0.9, to_radius = 1,
                 from_angle = 210, to_angle = 270) +
    hex_segment2(linewidth = 0, fill = paste0(red, "ff"), # bottom
                 from_radius = 0.9, to_radius = 1,
    ...
    geom_url("www.bioconductor.org", x = 0.98, y = 0.17,
             color = paste0(red, "ff"), size = 6.5, family = font_text) + 
    theme_sticker()
save_sticker(filename = "xcmsStash.png", hex)
